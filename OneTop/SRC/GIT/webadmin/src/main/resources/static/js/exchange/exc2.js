$(document).ready(function() {
    ...
    // 검색 버튼 이벤트
    $('#search_exc2_btn').bind('click', function () {
        $('#page').val('1');
        $('#search_exc2_form').submit();
        return false;
    });
...
if($('#sign_dt').attr('value') != undefined){
    var sList = $('#sign_dt').attr('value').split("-");
    var dList = [];
    ...
    $('#sign_dt').datepicker().data('datepicker').selectDate(dList);
}
...
/** Excel D/L click */
$("#exc2_excel_down_btn").click(function() {
    var f = document.search_exc2_form;
    ...
    post_to_url("exc2/excelDown", "_blank", params, "post");
});
